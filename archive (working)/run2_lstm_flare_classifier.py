...
# ----------------------------
SEQ_LEN = 200
STRIDE= 1 #SEQ_LEN/2 #50% overlap is common in signal processing
BATCH_SIZE = 256
EPOCHS = 10
LR = 1e-3
...
class FlarePhaseDataset(Dataset):
    ...
    """
    def __init__(self, lightcurve_dict, seq_len, tic_ids, stride=STRIDE):
        self.samples = []
        for tic in tic_ids:
            ...
            for i in range(0, len(flare_flux) - seq_len, int(stride)):
                ...
                # insist that both rise and decay are present in the sequence so the model is able to learn 
...
class LSTMFlareClassifier(nn.Module):
    ...
    """
    def __init__(self, input_size=1, hidden_size=128, num_layers=3, num_classes=3, use_attention=True):
        super().__init__()
...
def train_model(dataset_path=SYNTHETIC_PATH, batch_size=BATCH_SIZE, epochs=EPOCHS, learning_rate=LR, seq_len=SEQ_LEN, use_attention= True, input_size=1, hidden_size= 128, num_layers=3, num_classes=3, split_path= SPLIT_PATH, model_path= SAVE_PATH):
    """
